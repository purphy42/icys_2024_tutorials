...
def get_options():
    ...
    return local_query,setup,movie,user_config
...
def get_saddle_image_number_and_barriers(path):
    ...
    end_img = ('grep "energy without" OUTCAR | tail -n 1|cut -c 67-68')
...
def main():
    query,user_set_up_calculation,user_movie_setup,user_config = get_options() 
    NI = get_saddle_image_number_and_barriers(path_initial)
    if sys.argv[1] == "-l":
        query_structure = path_initial+"/0"+str(NI+1)+"/POSCAR"
    else:
        query_structure = sys.argv[1]
